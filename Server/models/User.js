...
UserSchema.statics.updatePIN = async function ( email, newPIN )
{
    try
    {
        ...
        if ( !isSamePIN )
        {
            throw new NotFoundError( "New PIN must be different from the old PIN" );
        }
...
UserSchema.statics.updatePasssword = async function ( email, newPassword )
{
    try
    {
        const user = await this.findOne( { email } );
        ...
        const isSamePassword = await bcrypt.compare( newPassword, user.login_pin );
        if ( !isSamePassword )
        {
            throw new NotFoundError( "New Password must be different from the old Password" );
        }
        ...
        await this.findOneAndUpdate(
            { email },
            { login_password: hashedPassword, wrong_password_attempts: 0, blocked_until_password: null }
        );
...
UserSchema.methods.comparePassword = async function ( candidatePassword )
{
    ...
    {
        throw new UnauthenticatedError( 'Invalid Login attempts exceeded. Please try after 30 minutes.' );
    }
